...
main (int argc, char** argv)
{
  ...
  for (const auto& point: *cloud)
  {
    ...
    file <<  std::hex <<(int16_t) (point.intensity)<<" "<< std::hex<<(int16_t) (point.z*100)<<" "<< std::hex<<(int16_t) (point.y*100)<<" "<< std::hex<<(int16_t) (point.x*100)<<"\n";
    std::stringstream aux_x;
      aux_x << std::hex << (int16_t) (point.x*100);
    std::stringstream aux_y;
      aux_y << std::hex << (int16_t) (point.z*100);
